...
class elevation_melt():
    ...
    def type_4(self,dx,dy,cenPt,basePt,basePln,baseSrf,outsidePt,colorF,colorW):
        ...
        rs.ObjectColor(frame,colorF)
        rs.ObjectColor(window,colorW)
        index=rs.ObjectMaterialI`ndex(frame)
        if index==-1:
            print "color!" 
            rs.MaterialColor(index,colorF)
            rs.AddMaterialToObject(frame)
...
def main():
    ...
    myEleMelt=elevation_melt()
    myEleMelt.generate(vals[0],vals[1],vals[3],vals[4])
    
    elevation=rs.GetObjects("select objects to transform",rs.filter.polysurface)
    #_transform(elevation)
    
    adv_transform(elevation,vals[5][0],vals[5][1],vals[5][2])
